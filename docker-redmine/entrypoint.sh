...
SMTP_CA_PATH=${SMTP_CA_PATH:-$REDMINE_ROOT_PATH/certs}
SMTP_CA_FILE=${SMTP_CA_FILE:-$REDMINE_ROOT_PATH/certs/ca.crt}
...
setup_configuration() {
  local fpath=$REDMINE_ROOT_PATH/config/configuration.yml
...
setup_database() {
  local fpath=$REDMINE_ROOT_PATH/config/database.yml
...
wait_db_postgres() {
  ...
  until PGPASSWORD=$DB_PASS psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c '\q';
  do
    echo "Waiting for database connection..."
    sleep 5
  done
...
if [ ! -e /init_done ]; then
  sudo -HEu $REDMINE_USER bundle exec rake generate_secret_token
  RAILS_ENV=production sudo -HEu $REDMINE_USER bundle exec rake db:migrate
  touch /init_done
  echo "##############################"
  echo "#"
